...
const ChatBot = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(`${PROD_LINK}/chat`, {
        prompt,
        conversationId,
      });
    ...
    if (showChatBot && chatHistory.length === 0) {
      const timer = setTimeout(() => {
        setChatHistory([
          {
            role: "bot",
            content: "Welcome to PollPal, how can I help you today?",
          },
        ]);
      }, 1000);
  ...
  const fetchEmojis = async () => {
    try {
      const response = await fetch(
        `https://emoji-api.com/emojis?access_key=${emojiApi}`
      );
            ...
            <>
              <div className="messages-container">
                <div className="disclaimer">
                  <p>
                    This chatbot is powered by OpenAI and may not always provide
